...
def _handle_calendarific(year, country_code, intent):
    IntegrationLogging.info("Handling integration calendarific")
    holidays = []
    response = ""
    ...
    try:
        if len(all_holidays.json()["response"]["holidays"]) > 1:
            IntegrationLogging.info("Multiple holidays found")
            for holiday in all_holidays.json()["response"]["holidays"]:
                IntegrationLogging.info(f"Found holiday {holiday['name']}")
            ...
            IntegrationLogging.info("returning response")
...
def _handle_home_assistant(tag, intent):
    IntegrationLogging.info("handling integration home assistant")
    try:
        IntegrationHomeAssistant.post_api_services_by_domain_by_service(intent["domain"], tag)
        IntegrationLogging.info(f"Executing {tag}")
...
def _handle_omdb(sentence, intent):
    IntegrationLogging.info("handling integration omdb")
    movie_name = ' '.join([word for word in str(sentence).split(" ") if word not in intent["patterns"]]).strip()
    IntegrationLogging.info(f"Found movie name {movie_name}")
    ...
    try:
        ...
        IntegrationLogging.info(f"Found information: {movie_data}")
...
def _handle_theaudiodb(sentence, intent):
    ...
    try:
        ...
        IntegrationLogging.info(f"Found artist data: {artist_data}")
...
def _handle_themealdb_or_thecocktaildb(sentence, intent):
    ...
    if meal_statuscode == 200:
        ...
        try:
            if len(meals) > 1:
                IntegrationLogging.info(f"Multiple meals found for {meal_name}")
                for meal in meals:
                                     ...
                                     "mealIngredients": ", ".join(
                                         [meal[mealIngredient] for mealIngredient in meal if
                                          str(mealIngredient).startswith("strIngredient") and meal[
                                              mealIngredient] != ''])}
                        IntegrationLogging.info(f"Meal found: {meal_data}")
            ...
            try:
                if cocktail_statuscode == 200:
                    if len(drinks) > 1:
                        IntegrationLogging.info(f"Multiple cocktails found for {cocktail_name}")
                        for drink in drinks:
                            if str(drink["strDrink"]).lower() == str(cocktail_name).lower():
                                ...
                                IntegrationLogging.info(f"Cocktail found: {drink_data}")
                                answer_option = choice(
...
class IntegrationAi:
    ...
    def load_intents(self):
        IntegrationLogging.info("Loading intents")
        for intent in self.intents:
            ...
            IntegrationLogging.info(f"loaded intent: {intent}")

    def check_sentence(self, sentence, received_from=None):
        IntegrationLogging.info(f"Checking started for {sentence}")
        ...
        for intent_name, intent_data in self.recognize_data.items():
            if received_from in intent_data["accessible_by"]:
                ...
                IntegrationLogging.info(f"Intersection of {intersection_value} found between {sentence} and {intent_data['patterns']}")
                specific_words_intersection = len(intent_data["specific_words"].intersection(set_sentence))
                IntegrationLogging.info(f"Intersection of {intersection_value} found between {sentence} and {intent_data['specific_words']}")

                if specific_words_intersection > 0:
                    IntegrationLogging.info(f"Old intersection value: {intersection_value}")
                    intersection_value += specific_words_intersection
                    IntegrationLogging.info(f"Intersection of specific words found and value added: {intersection_value}")

                if intersection_value > 0 and intersection_value > highest_intersection_value:
                    IntegrationLogging.info(f"New highest intersection found: {intent_data}")
        ...
        if highest_intersection_value > 0:
            IntegrationLogging.info(f"Executing highest intent: {highest_intersection_tag}, {highest_intent_data}, {sentence}")
            return _execute_intent(highest_intersection_tag, highest_intent_data, sentence)
        else:
            IntegrationLogging.critical("No intersection found")
            return {"forced_behaviour": True,
                    "answer": languagemodels.do(sentence),
                    }
